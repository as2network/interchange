...
async function createHubs(provider: Provider, [admin, user1]: Wallet[]) {
  ...
  const msgSenderExample = await new MsgSenderExampleFactory(admin).deploy(AddressZero);
...
describe("Proxy Account Forwarder", () => {
  it("Deploy proxy account and verify the correct address is computed.", async () => {
    ...
    const proxyAccountContract = new ProxyAccountFactory(user1).attach(computedProxyAddress);
  ...
  it("Sign a single meta-transaction with multinonce", async () => {
    const { msgSenderExample, proxyDeployer, admin } = await loadFixture(createHubs);
    ...
    expect(forwardParams._metaTx.to).to.eq(msgSenderExample.address, "Target contract");
  ...
  it("Encode a signed meta-transaction", async () => {
    const { msgSenderExample, proxyDeployer, user1 } = await loadFixture(createHubs);
    ...
    await expect(tx)
      .to.emit(msgSenderExample, msgSenderExample.interface.events.WhoIsSender.name)
  ...
  it("Deploy a new meta-contract with the ProxyAccountDeployer installed.", async () => {
    ...
    const initCode = new MsgSenderExampleFactory(admin).getDeployTransaction(proxyDeployer.address)
      .data! as string;
    ...
    const deployer = new DelegateDeployerFactory(admin).attach(DELEGATE_DEPLOYER_ADDRESS);
    const data = deployer.interface.functions.deploy.encode([initCode, 0, keccak256(extraData)]);
    ...
    const msgSenderExample = new MsgSenderExampleFactory(admin).attach(msgSenderExampleAddress);
  ...
  it("Deploy a new meta-contract with a balance of 1 ETH", async () => {
    ...
    const msgSenderAddress = forwarder.computeAddressForDeployedContract(initCode, extraData);
  ...
  it("Deploy the proxy contract and a meta-tx with MultiSend", async () => {
    const { proxyDeployer, admin, user1, msgSenderExample } = await loadFixture(createHubs);
  ...
  it("Send two transactions via call multisend. setting: bitflip", async () => {
    const { msgSenderExample, echo, proxyDeployer, admin, user1 } = await loadFixture(createHubs);
    ...
    await expect(tx).to.emit(echo, echo.interface.events.Broadcast.name).withArgs("hello");
  ...
  fnIt<ProxyAccountForwarder>(
    ...
    async () => {
      ...
      expect(forwarder.address).to.eq(ProxyAccountForwarder.getAddress(user1.address));
