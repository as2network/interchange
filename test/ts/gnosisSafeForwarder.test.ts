...
async function createSafe(provider: Provider, [admin, owner, sender]: Wallet[]) {
  const { proxyFactoryAddress, gnosisSafeAddress } = await deployMetaTxContracts(admin, false);

  const proxyFactory = new ProxyFactoryFactory(admin).attach(proxyFactoryAddress);

  const gnosisSafeMaster = new GnosisSafeFactory(admin).attach(gnosisSafeAddress);
...
describe(GnosisSafeForwarder.name, () => {
  ...
  it("Send a single transaction to an echo contract", async () => {
    ...
    expect(lastMessage, "Last message should be recorded in the echo contract as hello.").to.eq(
      "hello"
    );
  }).timeout(50000);
  ...
  it("Meta-deploy the echo contract", async () => {
    ...
    const data = echoInterface.functions.sendMessage.encode(["meta-deployed yay"]);
    ...
    expect(lastMessage, "Last message should be recorded in the echo contract.").to.eq(
      "meta-deployed yay"
    );
  }).timeout(50000);
  ...
  it("Decode a meta-transaction", async () => {
    const { sender, owner, proxyFactory, gnosisSafeMaster } = await loadFixture(createSafe);
    ...
    const gnosisSafe = new GnosisSafeFactory(sender).attach(gnosisForwarder.address);
    await (
    ...
    expect(await echoCon.functions.lastMessage(), "end last message").to.eq("hello");
  }).timeout(50000);
  ...
  it("Decode a meta-transaction batch", async () => {
    ...
    expect(decodedTx.to, "Sending to the echo contract").to.eq(MULTI_SEND_ADDRESS);
    expect(decodedTx.value.toString(), "Zero value sent").to.eq("0");
  ...
  fnIt<GnosisSafeForwarder>(
    ...
    async () => {
      const { sender, proxyFactory, gnosisSafeMaster } = await loadFixture(createSafe);
