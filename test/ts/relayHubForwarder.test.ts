...
async function createHubs(provider: Provider, [admin, user1, user2]: Wallet[]) {
  const { relayHubAddress, proxyAccountDeployerAddress } = await deployMetaTxContracts(admin);
  ...
  const msgSenderExample = await new MsgSenderExampleFactory(admin).deploy(relayHub.address);
...
const checkMetaTx = (
  ...
  expect(forwardParams._replayProtectionAuthority, "Nonce replay protection").to.eq(
...
describe("RelayHub Forwarder", () => {
  it("Sign a meta-transaction with multinonce and check the forward params are correct", async () => {
    ...
    const forwarder = new RelayHubForwarder(ChainID.MAINNET, admin, relayHub.address, replay);
    ...
    checkMetaTx(metaTx, forwarder, "0", "0", msgSenderExample.address, callData, replay.address);
  }).timeout(50000);
  ...
  it("Sign a meta-transaction with bitflip and check the forward params are correct", async () => {
    ...
    const forwarder = new RelayHubForwarder(ChainID.MAINNET, admin, relayHub.address, bitlip);
  ...
  it("Sign multiple meta-transactions with bitflip", async () => {
    ...
    const proxyForwarder = new RelayHubForwarder(ChainID.MAINNET, user2, relayHub.address, bitflip);
  ...
  it("Send one transaction via the batch. It should succeed.", async () => {
    ...
    const metaTxList = [{ to: msgSenderExample.address, data: callData, revertOnFail: false }];
  ...
  it("Send two transactions via the batch. First transaction reverts, but revert message is emitted.", async () => {
    const { msgSenderExample, admin, relayHub, echoCon } = await loadFixture(createHubs);
