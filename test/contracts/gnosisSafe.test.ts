...
async function deployProxy(
  ...
  const tx = await proxyFactory.createProxyWithNonce(gnosisSafeMaster.address, setup, salt);

  await tx.wait(1);

  const deployTx = new GnosisProxyFactory(owner).getDeployTransaction(gnosisSafeMaster.address);
...
async function prepareTransactionData(
  ...
  const TYPEHASH = "0xbb8310d486368db6bd6f849402fdd73ad53d316b5a4b2644ad6efe0f941286d8";
  ...
  const domainSeparator = keccak256(
    defaultAbiCoder.encode(
      ["bytes32", "address"],
      ["0x035aff83d86937d35b32e04f0ddc6ff469290eef2f1b692d8a815c89404d4749", proxyAddress]
    )
  );
...
describe("GnosisSafe", () => {
  fnIt<gnosisSafeFunctions>(
    ...
    async () => {
      const { provider, proxyFactory, gnosisSafeMaster, owner } = await loadFixture(createSafe);

      const proxyAddress = await deployProxy(gnosisSafeMaster, proxyFactory, owner);
  ...
  fnIt<proxyFactoryFunctions>(
    ...
    async () => {
      ...
      expect(sentBy, "Counter should be incremented by the multi-send address.").to.eq(
        MULTI_SEND_ADDRESS
      );
    }
  );
  ...
  fnIt<proxyFactoryFunctions>(
    ...
    async () => {
      ...
      const sentBy = await counter.lastSentBy();
      expect(c.eq(5), "Counter should be incremented 5 times").to.be.true;
      expect(sentBy, "Counter should be incremented by the proxy contract.").to.eq(proxyAddress);
    }
  );
