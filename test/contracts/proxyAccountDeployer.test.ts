...
async function createProxyAccountDeployer(provider: Provider, [admin, owner, sender]: Wallet[]) {
  ...
  const { proxyAccountDeployerAddress } = await deployMetaTxContracts(admin, false);
  const proxyDeployer = new ProxyAccountDeployerFactory(admin).attach(proxyAccountDeployerAddress);
...
describe("ProxyAccountDeployer", () => {
  fnIt<proxyDeployerFunctions>(
    ...
    async () => {
      const { proxyDeployer, sender } = await loadFixture(createProxyAccountDeployer);
  ...
  fnIt<proxyDeployerFunctions>(
    ...
    async () => {
      ...
      await proxyDeployer.connect(sender).createProxyAccount(sender.address);
      const tx = proxyDeployer.connect(sender).createProxyAccount(sender.address);
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      ...
      await expect(tx).to.emit(msgSenderCon, msgSenderCon.interface.events.WhoIsSender.name);
    }
  );
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      ...
      const proxyAccount = new ProxyAccountFactory(owner).attach(forwarder.address);
      ...
      await expect(tx).to.revertedWith("Owner did not sign this meta-transaction.");
    }
  );
  fnIt<accountFunctions>(
    (a) => a.forward,
    ...
    async () => {
      ...
      const proxyAccountContract = new ProxyAccountFactory(sender).attach(forwarder.address);
      ...
      await expect(tx).to.be.revertedWith("Owner did not sign this meta-transaction.");
    }
  );
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      const { proxyDeployer, owner, sender } = await loadFixture(createProxyAccountDeployer);
      ...
      const proxyAccount = new ProxyAccountFactory(sender).attach(forwarder.address);

      // Deploy the proxy using CREATE2
      const initCode = msgSenderFactory.getDeployTransaction(proxyDeployer.address).data! as string;
      ...
      await expect(tx)
        .to.emit(msgSenderExampleCon, msgSenderExampleCon.interface.events.WhoIsSender.name)
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      ...
      await expect(tx2).to.be.revertedWith("Multinonce replay protection failed");
    }
  );
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      const { msgSenderCon, proxyDeployer, admin } = await loadFixture(createProxyAccountDeployer);
      ...
      const encodedCallData = defaultAbiCoder.encode(
        ["uint", "tuple(address to, uint value, bytes data, bool revertOnFail, uint callType)[]"],
        [CallType.BATCH, metaTxList]
      );

      const encodedMetaTx = defaultAbiCoder.encode(
        ["bytes", "bytes", "address", "address", "uint"],
        [encodedCallData, replayProtection, AddressZero, proxyAccountAddress, ChainID.MAINNET]
      );

      const signature = await admin.signMessage(arrayify(keccak256(encodedMetaTx)));

      const proxyAccountInterface = new Interface(abi) as ProxyAccount["interface"];
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      ...
      const proxyAccount = new ProxyAccountFactory(admin).attach(forwarder.address);
      const tx = admin.sendTransaction({
        to: forwarder.address,
        data: encodedBatch,
      });
  ...
  fnIt<accountFunctions>(
    ...
    async () => {
      ...
      const revertMessageTester = await new RevertMessageTesterFactory(sender).deploy();

      const tx = revertMessageTester.testCallNoRevert(minimalTx.to, minimalTx.data);

      await expect(tx)
        .to.emit(revertMessageTester, revertMessageTester.interface.events.Info.name)
        .withArgs(
