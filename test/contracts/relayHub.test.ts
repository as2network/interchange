...
async function createRelayHub(provider: Provider, [admin, owner, sender]: Wallet[]) {
...
describe("RelayHub Contract", () => {
  fnIt<relayHubFunctions>(
    ...
    async () => {
      ...
      expect(relayhub_bitflip).to.eq("0x0000000000000000000000000000000000000001");
    }
  );
  ...
  fnIt<relayHubFunctions>(
    ...
    async () => {
      const { relayHub, owner, sender, msgSenderCon } = await loadFixture(createRelayHub);
  ...
  fnIt<relayHubFunctions>(
    ...
    async () => {
      const { relayHub, owner, sender, msgSenderCon, forwarderFactory } = await loadFixture(
        createRelayHub
      );
      const msgSenderCall = msgSenderCon.interface.functions.test.encode([]);
      const value = new BigNumber("0");
      const encodedReplayProtection = "0x";
      const replayProtectionAuthority = "0x0000000000000000000000000000000000000002";
  ...
  const encodeAndsign = async (
    ...
    const encodedMetaTx = defaultAbiCoder.encode(
      ...
      [encodedData, replayProtection, replayProtectionAuthority, forwarderAddress, ChainID.MAINNET]
    );
  ...
  fnIt<relayHubFunctions>(
    ...
    async () => {
      ...
      const msgSenderCall = msgSenderCon.interface.functions.test.encode([]);
      const value = new BigNumber("0");
      const encodedReplayProtection = defaultAbiCoder.encode(["uint", "uint"], [0, 123]);
      ...
      // An empty revert message, since the function doesn't exist on that contract address
      await expect(tx).to.be.revertedWith("Multinonce replay protection failed");
    }
  );
  ...
  fnIt<relayHubFunctions>(
    ...
    async () => {
      const { relayHub, owner, sender, msgSenderCon, bitFlipNonceStore } = await loadFixture(
        createRelayHub
      );
      const msgSenderCall = msgSenderCon.interface.functions.test.encode([]);
  ...
  fnIt<relayHubFunctions>(
    ...
    async () => {
      const { msgSenderCon, admin, relayHub } = await loadFixture(createRelayHub);
      ...
      const metaTxList = [{ to: msgSenderCon.address, data: msgSenderCall, revertOnFail: false }];
  ...
  fnIt<relayHubFunctions>(
    ...
    async () => {
      const { msgSenderCon, admin, relayHub, echoCon } = await loadFixture(createRelayHub);
      const forwarder = new RelayHubForwarder(
        ChainID.MAINNET,
        admin,
        RELAY_HUB_ADDRESS,
