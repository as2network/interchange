...
async function signCall(
  ...
  const encodedReplayProtection = defaultAbiCoder.encode(["uint", "uint"], [nonce1, nonce2]);
...
async function createReplayProtection(provider: Provider, [admin, owner, sender]: Wallet[]) {
  const replayProtection = await new ReplayProtectionWrapperFactory(admin).deploy();
...
describe("ReplayProtection", () => {
  fnIt<replayProtection>(
    ...
    async () => {
      const { replayProtection, admin } = await loadFixture(createReplayProtection);
  ...
  fnIt<replayProtection>(
    ...
    async () => {
      ...
      const nonceReplayProtection = defaultAbiCoder.encode(["uint", "uint"], [0, 0]);

      await replayProtection.noncePublic(admin.address, nonceReplayProtection);

      const index = keccak256(
        defaultAbiCoder.encode(["address", "uint", "address"], [admin.address, 0, AddressZero])
      );
  ...
  fnIt<replayProtection>(
    ...
    async () => {
      ...
      const queue = 0;
      for (let i = 0; i < 50; i++) {
        const nonceReplayProtection = defaultAbiCoder.encode(["uint", "uint"], [queue, i]);
  ...
  fnIt<replayProtection>(
    ...
    async () => {
      ...
      for (let queue = 0; queue < 50; queue++) {
        const nonceReplayProtection = defaultAbiCoder.encode(["uint", "uint"], [queue, queueNonce]);
  ...
  fnIt<replayProtection>(
    ...
    async () => {
      ...
      await replayProtection.bitflipPublic(admin.address, nonceReplayProtection);

      const index = keccak256(
        defaultAbiCoder.encode(
          ["address", "uint", "address"],
          [admin.address, bitmapIndex, "0x0000000000000000000000000000000000000001"]
        )
      );
  ...
  fnIt<replayProtection>(
    ...
    async () => {
      ...
      await expect(tx)
        .to.emit(replayProtection, replayProtection.interface.events.ReplayProtectionInfo.name)
