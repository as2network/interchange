...
export class GnosisSafeForwarder
  ...
  constructor(
    ...
    super(
      ...
      GnosisSafeForwarder.getAddress(signerAddress, chainID, {
        proxyFactoryAddress: options?.proxyFactoryAddress || PROXY_FACTORY_ADDRESS,
        gnosisSafeMasterAddress: options?.gnosisSafeMaster || GNOSIS_SAFE_ADDRESS,
      }),
  ...
  private defaultCallData(data: ProxyAccountCallData): Required<ProxyAccountCallData> {
    return {
      to: data.to,
  ...
  protected async signAndEncodeMetaTransaction(data: ProxyAccountCallData): Promise<MinimalTx> {
    ...
    const signature = await this.hashAndSign(dataWithDefaults, replayProtection);
  ...
  public async hashAndSign(callData: Required<ProxyAccountCallData>, replayProtection: string) {
    ...
    const domainSeparator = keccak256(
      defaultAbiCoder.encode(
        ["bytes32", "address"],
        ["0x035aff83d86937d35b32e04f0ddc6ff469290eef2f1b692d8a815c89404d4749", this.address]
      )
    );
  ...
  public async getWalletDeployTransaction(): Promise<MinimalTx> {
    ...
    const salt = keccak256(defaultAbiCoder.encode(["uint", "string"], [this.chainID, "as2ng"]));

    const callData = this.proxyFactory.interface.functions.createProxyWithNonce.encode([
      this.gnosisSafeMaster.address,
      setup,
      salt,
    ]);
  ...
  public static getAddress(
    ...
    const gnosisSafeInterface = new GnosisSafeFactory().interface as GnosisSafe["interface"];
    ...
    const salt = keccak256(defaultAbiCoder.encode(["uint", "string"], [chainId, "as2ng"]));
   ...
   */
  public computeAddressForDeployedContract(initData: string, extraData: string): string {
