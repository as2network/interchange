...
import { defaultAbiCoder, BigNumberish, arrayify, keccak256 } from "ethers/utils";
...
> {
  ...
  public async signMetaTransaction(
    tx: XOR<TCallData, TDeployCallData> | XOR<TBatchCallData, TBatchDeployCallData>[]
  ): Promise<MinimalTx> {
    if (Array.isArray(tx)) {
  ...
  protected encodeForDeploy(initCode: string, extraData: string, value: BigNumberish) {
    const deployer = new DelegateDeployerFactory(this.signer).attach(DELEGATE_DEPLOYER_ADDRESS);
   ...
   */
  protected abstract async signAndEncodeMetaTransaction(data: TCallData): Promise<MinimalTx>;
...
export abstract class MiniForwarder<
  ...
  TBatchCallData extends CallData,
  TBatchDeployCallData extends CallData
> extends Forwarder<TCallData, TDeployCallData, TBatchCallData, TBatchDeployCallData> {
   ...
   * @param data ProxyAccountCallData or RelayCallData
   */
  protected async signAndEncodeMetaTransaction(data: TCallData): Promise<MinimalTx> {
    ...
    const signature = await this.signer.signMessage(arrayify(keccak256(encodedMetaTx)));
