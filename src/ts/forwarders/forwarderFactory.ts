...
export abstract class ForwarderFactory<D> {
  ...
  // we use 'as2' here since a static prop cant have access
  // to the instance types eg. ProxyAccountForwarderFactory
  private static cache: Map<string, as2> = new Map();
  ...
  protected async getCacheId(
    ...
    return `${this.type}:${chainid}:${replayProtectionType}:${await signer.getAddress()}`;
  }
  ...
  public async create(
    ...
    const cacheId = await this.getCacheId(chainId, replayProtectionType, signer);

    if (!ForwarderFactory.cache.get(cacheId)) {
      const forwarder = await this.createNew(chainId, replayProtectionType, signer);
