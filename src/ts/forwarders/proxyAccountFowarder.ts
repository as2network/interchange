...
import { ProxyAccountDeployer, ProxyAccountFactory, ProxyAccount } from "../../typedContracts";
...
import { PROXY_ACCOUNT_DEPLOYER_ADDRESS, BASE_ACCOUNT_ADDRESS } from "../../deployment/addresses";
...
export interface RevertableProxyAccountDeployCallData extends ProxyAccountDeployCallData {
  revertOnFail?: boolean;
}
...
export class ProxyAccountForwarder
  ...
  constructor(
    ...
    this.proxyDeployer = new ProxyAccountDeployerFactory(signer).attach(proxyDeployerAddress);
  }
  ...
  public decodeTx(data: string) {
    const proxyAccount = new ProxyAccountFactory(this.signer).attach(this.address);
  ...
  public decodeBatchTx(data: string) {
    ...
    const functionArgs: {
      ...
      _metaTxList: parsedTransaction.args[0].map((a: as2) => ({
        to: a[0],
        value: a[1],
        data: a[2],
        revertOnFail: a[3],
  ...
  protected encodeBatchCallData(txBatch: RevertableProxyAccountCallData[]): string {
