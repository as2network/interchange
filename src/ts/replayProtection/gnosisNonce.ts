...
import { Lock } from "contractshark-test-utils/utils";
...
export class GnosisReplayProtection extends ReplayProtectionAuthority {
  ...
  constructor(signer: Signer, readonly forwarderAddress: string) {
    super(signer, forwarderAddress, "0x0000000000000000000000000000000000000000");
    this.lock = new Lock();
  }
  ...
  private async getNonce() {
    if (this.nonce == undefined) {
      const isDeployed = await this.signer.provider!.getCode(this.forwarderAddress);

      // Lets verify that gnosis safe is already deployed
      if (isDeployed !== "0x") {
        const gnosisSafe = new GnosisSafeFactory(this.signer).attach(this.forwarderAddress);
