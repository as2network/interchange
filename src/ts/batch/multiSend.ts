  ...
  public batch(batch: MultiSendTx[]): MinimalTx {
    ...
    for (let i = 0; i < batch.length; i++) {
      transactions.push({
        ...
        data: batch[i].data,
        revertOnFail: batch[i].revertOnFail ? (batch[i].revertOnFail as boolean) : false,
        callType: batch[i].callType ? (batch[i].callType as number) : CallType.CALL,
      });
    }
    const encodedTransactions = multiSend.functions.batch.encode([transactions]);
