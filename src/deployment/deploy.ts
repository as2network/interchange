...
import { keccak256, toUtf8Bytes, getCreate2Address, parseEther } from "ethers/utils";
...
async function deployContract(
  ...
  const gas = (await contractFactory.signer.provider!.estimateGas(deployTx)).toNumber() * 1.2;
...
export const deployMetaTxContracts = async (admin: Signer, logProgress: boolean = false) => {
  ...
  const relayHubSalt = keccak256(toUtf8Bytes(VERSION + "|" + RELAY_HUB_SALT_STRING));
  ...
  const delegateDeployerSalt = keccak256(toUtf8Bytes(VERSION + "|" + MULTI_SEND_SALT_STRING));
  const delegateDeployerAddress = await deployContract(
    deployerContract,
    delegateDeployerFactory,
    delegateDeployerSalt
  );
  logProgress && console.log("DelegateDeployer address: " + delegateDeployerAddress);

  const multiSendFactory = new MultiSendFactory(admin);
  const multiSendSalt = keccak256(toUtf8Bytes(VERSION + "|" + MULTI_SEND_SALT_STRING));
  ...
  const gnosisSafeSalt = keccak256(toUtf8Bytes(VERSION + "|" + GNOSIS_SAFE_SALT_STRING));
  ...
  const proxyFactorySalt = keccak256(toUtf8Bytes(VERSION + "|" + PROXY_FACTORY_SALT_STRING));
